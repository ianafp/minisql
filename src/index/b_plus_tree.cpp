...
bool BPLUSTREE_TYPE::Insert(const KeyType &key, const ValueType &value, Transaction *transaction) {
  if (this->root_page_id_ == INVALID_PAGE_ID) {
    ...
    return true;
  } else return InsertIntoLeaf(key,value);
}
/*
...
bool BPLUSTREE_TYPE::InsertIntoLeaf(const KeyType &key, const ValueType &value, Transaction *transaction) 
{
  ...
  return false;
    ...
    // need to split
      B_PLUS_TREE_LEAF_PAGE_TYPE *new_leaf_page = Split(root_leaf_page);
  ...
  // the leaf if full, split
    B_PLUS_TREE_LEAF_PAGE_TYPE *new_leaf_ = Split<B_PLUS_TREE_LEAF_PAGE_TYPE>(leaf_page_);
  // insert to parent
...
void BPLUSTREE_TYPE::InsertIntoParent(BPlusTreePage *old_node, const KeyType &key, BPlusTreePage *new_node,Transaction *transaction) {
    B_PLUS_TREE_INTERNAL_PAGE_TYPE *parent_page = reinterpret_cast<B_PLUS_TREE_INTERNAL_PAGE_TYPE *>(
    ...
    // internal node full after insert, need to split
      B_PLUS_TREE_INTERNAL_PAGE_TYPE *new_internal_page = Split<B_PLUS_TREE_INTERNAL_PAGE_TYPE >(parent_page);
    if (parent_page->IsRootPage()) {
    // need to generate new root
        int new_root_id;
        // new root
      B_PLUS_TREE_INTERNAL_PAGE_TYPE *new_root_internal = reinterpret_cast<B_PLUS_TREE_INTERNAL_PAGE_TYPE *>(buffer_pool_manager_->NewPage(new_root_id));
        ...
        new_root_internal->InsertNodeAfter(parent_page->GetPageId(), new_internal_page->KeyAt(0),
...
// need to find the leaf contain the key
  ...
  while (!temp_internal_ptr->IsLeafPage()) {
   // unpin this page
    ...
    temp_internal_ptr = reinterpret_cast<B_PLUS_TREE_INTERNAL_PAGE_TYPE *>(buffer_pool_manager_->FetchPage(temp_internal_ptr->ValueAt(child_index)));
  }
  // find leaf
...
bool BPLUSTREE_TYPE::CoalesceOrRedistribute(N *node, Transaction *transaction) {
  ...
  if (index==0)
   ...
   else sibling_page = reinterpret_cast<N *>(buffer_pool_manager_->FetchPage(parent_page->ValueAt(index-1)));
