...
CatalogManager::CatalogManager(BufferPoolManager *buffer_pool_manager, LockManager *lock_manager,
  ...
  buf = this->buffer_pool_manager_->FetchPage(1)->GetData();
  this->buffer_pool_manager_->UnpinPage(1,false);
  TableHeap* table_heap = reinterpret_cast<TableHeap*>(buf);
...
dberr_t CatalogManager::CreateTable(const string &table_name, TableSchema *schema,
  ...
  page_id_t root_page;
  this->buffer_pool_manager_->FetchPage(root_page);
  ...
  TableHeap* table_heap = reinterpret_cast<TableHeap*>(this->buffer_pool_manager_->FetchPage(1)->GetData());
  this->buffer_pool_manager_->UnpinPage(1);
  table_info->Init(table_meta,table_heap);
...
dberr_t CatalogManager::CreateIndex(const std::string &table_name, const string &index_name,
  ...
  if(it!=this->index_names_.end()) return DB_INDEX_NOT_FOUND;
...
dberr_t CatalogManager::DropTable(const string &table_name) {
  ...
  auto(auto t:it_index){
    index_id.push_back(it->second);
  }
  this->index_names.erase(table_name);
  for(auto m:index_id){
